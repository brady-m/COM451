...
#define BackgroundRed 0.1f
#define BackgroundGreen 0.1f
#define BackgroundBlue 0.1f

#define AttractorRed 0.5f
#define AttractorGreen 0.01f
#define AttractorBlue 1.0f

#define zInitialSize 6
#define zScale 1.2f
...
GPU_Palette initGPUPalette(unsigned int imageWidth, unsigned int imageHeight)
{
    ...
    cudaMalloc((void**)&X.green, X.num_pixels * sizeof(float));
    if (err != cudaSuccess) {
        printf("cuda error allocating green = %s\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
    cudaMalloc((void**)&X.blue, X.num_pixels * sizeof(float));
    if (err != cudaSuccess) {
